...
@section('content')
    ...
    <form class="row maskking-form" id="money_receipt_form">
        ...
        <!--Hidden Inputs-->
        <input type="hidden" name="money_receipt_client_id" value="{{$invoiceData['client_id']}}">
        <input type="hidden" name="money_receipt_id" id="money_receipt_id" value="{{$moneyReceiptData->money_receipt_id}}">
        <input type="hidden" name="money_receipt_client_transaction_id" value="{{$moneyReceiptData->money_receipt_client_transaction_id}}">
        <input type="hidden" name="money_receipt_account_transaction_id" value="{{$moneyReceiptData->money_receipt_account_transaction_id}}">
                            ...
                            <div class="col-lg-6 col-sm-12">
                                <span class="float-label">
                                    <input type="text" class="form-control form-control-lg" id="searchClient"
                                        placeholder="Search Client" name="client_name" value="{{$invoiceData['client_name']}}" readonly style="background: #ededed;">                                       
                            ...
                            <div class="col-lg-6 col-sm-12">
                                <span class="float-label">
                                    <input type="text" class="form-control form-control-lg"id="searchInvoice"
                                        placeholder="Invoice" name="invoice_no" value="{{$invoiceData['invoice_no']}}" readonly style="background: #ededed;">                                       
                                    <label class="form-label" for="TextInput">Invoice</label>
                                </span>
                                        ...
                                        <table class="meta">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td><textarea class="form-control" name="money_receipt_voucher_no">{{$MoneyReceipt->generate_vouchar_no()}}</textarea></td>
                                                </tr>
                                                <tr>
                                    ...
                                    <table class="items">
                                        ...
                                        <div id="billingDetailsList">
                                            <tr class="item-row billing-details-row">
                                                <td class="item-name">
                                                    <div class="col-12">
                                                <textarea name="money_receipt_invoice_no" id="money_receipt_invoice_id" readonly>{{$invoiceData['invoice_no']}}</textarea>
                                                       

                                                    </div>
                                                </td>
                                                <td class="description">
                                                    <select class="form-control form-control-lg custom-select" name="money_receipt_account_id">
                                                         <option value="" disabled selected>
                                                                Select Account
                                                            </option>  
                                                        @foreach($MoneyReceipt->list_of_account() as $row)        
                                                            <option value="{{$row->account_id}}" @if($row->account_id == $selectedAccount[0]->transaction_account_id) selected @endif>{{$row->account_name.', '.$row->account_number}}</option>
                                                            @endforeach
                                                        </select>
                                                </td>
<?php
...
?>
                                                <td class="description">
                                                    <textarea name="money_receipt_invoice_net_total" id="money_receipt_invoice_net_total" readonly>{{$invoiceData['grand_total']}}</textarea>
                                                </td>


                                                <td><textarea class="cost" name="money_receipt_invoice_due_amount" readonly>{{$total_due}}</textarea></td>
                                                <input type="hidden" class="cost " data-row="1" id="money_receipt_old_amount" name="money_receipt_old_amount">{{$moneyReceiptData['money_receipt_total_amount']}}</input>
                                                <td><textarea class="cost " data-row="1" id="money_receipt_amount" name="money_receipt_amount">{{$moneyReceiptData['money_receipt_total_amount']}}</textarea></td>
                                                <input type="hidden" class="qty " id="money_receipt_old_discount" name="money_receipt_old_discount" data-row="1">{{$moneyReceiptData['money_receipt_total_discount']}}</input>
                                                <td><textarea class="qty " id="money_receipt_discount" name="money_receipt_discount" data-row="1">{{$moneyReceiptData['money_receipt_total_discount']}}</textarea></td>
                                                <td><input type="date" class="qty" data-row="1" id="money_receipt_date" name="money_receipt_date" value="{{$moneyReceiptData['money_receipt_payment_date']}}"></td>
                                            </tr>
                                        </div>
                                        </tbody>
                                    </table>
                    ...
                    <div class="col-12 text-center text-md-end">
                        <a type="button" class="btn btn-lg btn-secondary" href="{{url('invoice/'.$invoiceData['invoice_no'])}}"><i class="fa fa-envelope me-2"></i>SKIP</a>
...
$(this).parent().parent().remove();
    ...
    <script type="text/javascript">          
        ...
        $("#money_receipt_form").submit(function(e) {
                ...
                $moneyReceiptData = $('#money_receipt_id').val();
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    method: "POST",
                    url: "{{ url('money-receipt') }}/"+$moneyReceiptData,
                    data: data,
                    cache: false, 
                    contentType: false,
                    processData: false,               
...
//
//                        })
                        window.location.href = "{{ url('money-receipt') }}/"+data.moneyReceiptId;
                    }
                }).done(function() {
