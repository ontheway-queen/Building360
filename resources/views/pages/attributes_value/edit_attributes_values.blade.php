                                    ...
                                    <span class="float-label">
                                        <select class="form-control form-control-lg custom-select"
                                            ...
                                            @if ( $category->attributes_id ==  $attributevalue->attributes_id)
                                                 <option value="{{$category->attributes_id }}" selected>{{$category->attributes_name}}</option>
                                            @endif
                                               <option value="{{$category->attributes_id }}">{{$category->attributes_name}}</option>
                                            @endforeach
                                ...
                                <div class="col-lg-4 col-md-6 col-sm-6">
                                    <span class="float-label">
                                        <input type="text" class="form-control form-control-lg" id="attributes_name"
                                            placeholder="Attributes Name" name="attribute_value_name" value="{{$attributevalue->attributes_value}}">
        ...
        $("#attributes_form").submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    window.location.href = "{{ url('attributes-values') }}";
                }
            }).done(function() {
